 ...
 *
 * TBD: Implement two's complement for division with one negative and one positive number
 *
 * Sanjana Shah
 * August 13, 2017
 */
...
class DecToPosit {
	...
	public:
		...
		bitset<32> positFormat(short s, int k, int expVal, double frac) {
			//Keep adding to a string, then go character by character and turn it into a bitset
			...
			double quotient = frac;
			int rem;

			if(quotient != 0) {
				while(quotient != 1 && shift > 0) {
					quotient *= 2;
					if(quotient > 1) {
						quotient -= 1;
						rem = 1;
					...
					cout << temp4 << " " << quotient << " " << shift << endl;
				}
				temp = 1 << (shift+1);
				for(int i = 0; i < 32; i++)
					temp4[i] = temp4[i] || temp[i];
			...
			//Convert string into into bitset
			for(int i = 31; i >= 0; i--) {
				if(posString.at(i) == '0') {
					y[31-i] = 0;
				} else {
...
int main() {
	...
	Division div;
	div.setValues(s1, reg1, expVal1, frac1, s2, reg2, expVal2, frac2);
    ...
    //REGIME
	bitset<REGBITS> regime = div.createReg(dtp.getK() - dtp2.getK());
	cout << "Subtract k's for regime " << regime << endl;

    //EXPONENT
	bitset<ESBITS+1> exponent = expVal1.to_ulong() - expVal2.to_ulong();
	int exp = exponent.to_ulong();
	if(exp < 0) {
		cout << "1. Exp is less than representable range" << endl;
		regime = div.createReg(dtp.getK() - dtp2.getK() - 1);
		exponent = exponent.to_ulong() + USEED;
	}
	cout << "Subtract expVals for exponent, check if less than 0 -> shift from regime " << exponent << endl;
	...
	//Implicit number is 0. Keep multiplying fraction and pull form exponent and regime
	if(sum < 1) {
		sum *= 2;
		exp -= 1;
		exponent = exp;
	}
	bitset<FRACBITS> fracTemp = div.mantToBin(sum);
